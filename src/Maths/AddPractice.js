...
function AddPractice() {
  ...
  const handleAnswer = (event) => {
    ...
    if (parseInt(answer) === sum) {
      setMessage("Correct! You Have Got This");
    ...
    <>
      <div className="bg-image w-screen h-screen flex flex-col items-center">
        <section className="flex w-full flex-col py-14 space-y-14 items-center text-center">
          ...
          <span className="text-3xl bg-gradient-to-r from-violet-800 via-yellow-700 to-red-600 inline-block text-transparent bg-clip-text font-bold">PRACTICE</span>
        </section>
        <div className="flex flex-col items-center justify-center h-3/5 w-4/5 bg-slate-100 border-4 border-amber-400 font-sans italic">
          {answerSubmitted === false &&
            <div className="flex flex-col items-center justify-center space-y-4">
              <p className="text-3xl font-bold">What is the sum of <span className="text-orange-600">{num1}</span> and <span className="text-orange-600">{num2}</span>?</p>
              ...
              <form className='flex flex-col items-center justify-center space-y-5' onSubmit={handleAnswer}>
                ...
                <input type="text" className="text-xl font-sans italic w-full py-2 rounded-md bg-gray-300 border-2 border-indigo-500 focus:outline-none pl-16" placeholder="Enter the answer" value={answer} onChange={(e) => setAnswer(e.target.value)} />
