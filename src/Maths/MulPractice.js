    ...
    <>
      <div className="bg-image w-screen h-screen flex flex-col items-center">
        <section className="flex w-full flex-col py-14 space-y-14 items-center text-center">
          ...
          <span className="text-3xl bg-gradient-to-r from-violet-800 via-yellow-700 to-red-600 inline-block text-transparent bg-clip-text font-bold">MULTIPLICATION PRACTICE</span>
        </section>
        <div className="flex flex-col items-center justify-center h-3/5 w-4/5 bg-slate-100 border-4 border-amber-400 font-sans italic">
          {answerSubmitted === false &&
            <div className="flex flex-col items-center justify-center space-y-4">
              <p className="text-3xl font-bold">What is the Product of <span className="text-orange-600">{num1}</span> and <span className="text-orange-600">{num2}</span>?</p>
              ...
              <form className='flex flex-col items-center justify-center space-y-5' onSubmit={handleAnswer}>
                ...
                <input type="text" className="text-xl font-sans italic w-full py-2 rounded-md bg-gray-300 border-2 border-indigo-500 focus:outline-none pl-16" value={answer} onChange={(e) => setAnswer(e.target.value)} />
          ...
          {showScore && (
            <p className="text-4xl font-bold">
              Your score is: <span className="text-green-600">{score}</span> out of <span className="text-red-600">{questionsAnswered}</span>
            </p>
          )}
