        ...
        <>
            <div className="bg-image w-screen h-screen flex flex-col items-center">{/*header*/}
                <section className="flex w-full flex-col py-12 space-y-12 items-center text-center">
                    ...
                    <span className="text-2xl bg-gradient-to-r from-violet-800 via-yellow-700 to-red-600 inline-block text-transparent bg-clip-text font-bold">SUBRACTION EXAMPLE</span>
                </section>
                <div className='h-3/5 mx-20 w-3/5 rounded-xl border-4 border-amber-400 shadow-xl bg-teal-100 flex items-center justify-center space-x-20'>
                    <div className="border-4 border-gray-400 p-4 h-52 w-52 flex flex-col items-center justify-center space-y-3">
                        ...
                        <button className="bg-blue-500 text-white text-xl h-10 w-16 rounded italic hover:bg-pink-600" onClick={handleSubraction}>Sub</button>
                    </div>
                    {buttonClicked && <div class="border-4 border-gray-400 p-5 h-60 w-56 flex flex-col items-center justify-center space-y-3">
                        {result &&
                            <>
                                {borrows.length > 0 && <p className="text-2xl font-medium">Borrow: <span className="text-indigo-700">{borrows.join(", ")}</span></p>}
                                <p className="text-2xl font-medium">Number 1: <span className="text-red-600">{num1}</span></p>
                                <p className="text-2xl font-medium">Number 2: <span className="text-red-600">{num2}</span></p>
